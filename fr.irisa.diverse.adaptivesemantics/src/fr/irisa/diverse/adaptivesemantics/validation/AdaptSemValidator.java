...
public class AdaptSemValidator extends AbstractAdaptSemValidator {
	...
	public void validConclusionPatternmatching(Conclusion conclusion) {
		...
		validRuleInput(conf, semanticDomain);
	}
	
	private void validRuleInput(DefConfiguration conf, EPackage semanticDomain) {
		for(TermDef term : conf.getChilds()) {
			if(term instanceof DefConfiguration) {
				...
				if(! concept.getEPackage().equals(semanticDomain)) {
					...
					validRuleInput((DefConfiguration) term, semanticDomain);
				}
			}
		}
	}
...
//	@Check
//	public void validPremisePatternmatching(Premise premise) {
//		RefConfiguration conf = premise.getFrom();
//		Model m = (Model) premise.eContainer().eContainer().eGet(AdaptivesemanticsPackage.Literals.ADAPTIVE_SEMANTICS__MODEL);
//		EPackage semanticDomain = m.getSemanticdomain();
//			
//		validRuleInput(conf, semanticDomain);
//	}
